...
| `MAX_NUM_BATCHED_TOKENS` | **8192** | Evaluated in Issue [#28](../../issues/28) ([`20260827-issue28-speed-bt16384`](../results/20260827-issue28-speed-bt16384/)): 16384 yields +11.5% decode at 262K (51.4 tok/s), but **degrades deep TTFT by +22% at 131K (92.5s vs 75.7s)** and **+29% at 262K (228.6s vs 177.3s)**. Mechanism **uncharacterized**: the 235 MB bus-saturation story is withdrawn as arithmetically infeasible (~0.9 ms/layer-pass at 273 GB/s cannot account for +16.7 s) and was never measured. **Confounded**: that arm also ran `MAX_MODEL_LEN=460800` vs 1048576. The +11.5% decode sits inside a 32.2% run spread (37.18-53.72, n=7). 8192 stands regardless: 16384 lost deep TTFT, lost the starvation probe, and cost 21% of KV | ⚠️ vLLM's log suggests 16384. That advice assumes intra-node NVLink. On unified memory and multi-node RoCE it slows prefill |
