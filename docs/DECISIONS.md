...
|---|---|---|---|
| serving engine | **`eugr-spark-vllm-b12x`** (digest 7dc02f16, vLLM main); `anemll-v0.25.1` retired to reference arm, `dsv4.service` disabled — user decision 2026-08-30 | Arm-1 gate: garble ALL CLEAN, RULER-lite 16/16, deep-context 8/8, tools 6/7; c=4 +41 %, c=8 +20 % aggregate vs anemll ([`ENGINE-AB-3NODE.md`](ENGINE-AB-3NODE.md), [bundle](../results/20260830T194550Z-engine-ab-eugr/)) | ⚠️ Not yet a systemd unit and the gateway route (sparkmain:8100, old served name) is dead until the next boot serves both names on 8100 — see [handoff](HANDOFF-2026-08-30-ENGINE-AB.md) |
| speculative depth on the eugr engine | **OPEN** — arm 1 ran the recipe's dspark nst=5; the c=16 cliff (TTFT 7.0 s) is the draft-slot budget | Sweep nst ∈ {2,3,5,7} × c ∈ {1,4,8,16} pending ([handoff §3](HANDOFF-2026-08-30-ENGINE-AB.md)) | Anemll precedent: K=2 won at concurrency, K=5 single-stream ([#32](../../issues/32)); do not carry either number across engines unmeasured |
...
| TP=3 padding patch | **required on `anemll-v0.25.1`; MUST NOT be applied on `eugr-spark-vllm-b12x`** (native virtual-TP already pads 64→72 / 8→9; correctness verified 2026-08-30, [`ENGINE-AB-3NODE.md`](ENGINE-AB-3NODE.md)) | Correctness 14/14; [`patch.md`](patch.md) | ☠️ **Silently serves fluent nonsense.** Stock vLLM computes `8 // 3 == 2` and drops 6 of 8 attention groups |
