#!/usr/bin/env bash
# qwen-boot-dflash-tp2.sh — launch Qwen 3.8 + DFlash2 on 2 nodes (TP=2: sparkmain + spark1)
set -euo pipefail

MNBT="${1:-8192}"
LOG="${2:-$HOME/qwen-dflash-tp2-mnbt${MNBT}.log}"
...
RECIPE="qwen3.8-27b-dflash-tp2"

echo "=== qwen-boot-dflash-tp2: mnbt=$MNBT nodes=$NODES ==="
echo "    recipe: $RECIPE"
echo "    log:    $LOG"
