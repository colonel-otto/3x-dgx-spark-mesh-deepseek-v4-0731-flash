...
# RECONSTRUCTION -- see PROMPT PROVENANCE above. Not the original text.
DENSE_PROSE = (
    "Consider how a distributed inference system balances latency against "
    "throughput when many independent callers arrive at once, and describe "
    "the tradeoffs a scheduler must weigh when it decides which pending "
    "request to admit next rather than continuing the one already running."
)
...
def stream_one(base, model, prompt, max_tokens, out, idx, timeout=3600):
    body = {
        "model": model,
...
def single(base, model, prompt, max_tokens, reps, label):
    rows = []
    for i in range(reps):
        out = {}
        stream_one(base, model, prompt, max_tokens, out, 0)
        r = out[0]
        if "error" in r:
