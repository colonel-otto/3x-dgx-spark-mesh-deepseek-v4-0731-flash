...
def stream_one(base_url, model, prompt, max_tokens):
    ...
    # HARDENING (issue #26 lineage): the single-stream harness
    # (decode_depth_sweep.py) found that without an assertion, a model that
    # stops early under a short-answer-inviting prompt silently collapses the
    # decode window (25-26 tokens against a requested 256) and the resulting
    # tok/s is nonsense-fast. min_tokens=max_tokens+ignore_eos SHOULD make
    # that impossible, but verify rather than trust: fail loudly if the
    # server ever returns a different completion_tokens count.
    if output_tokens != max_tokens:
        raise RuntimeError(
            "decode window is %d tokens, expected %d -- min_tokens/ignore_eos "
            "were not honoured. Do not trust this sample (issue #26)."
...
async def run_case(base_url, model, target_prompt_tokens, concurrency, nonce_base, max_tokens):
    ...
    started = time.perf_counter()
    # gather(..., return_exceptions=True): one bad stream must not silently
    # vanish from the aggregate -- surface it as a hard failure instead.
    raw_results = await asyncio.gather(
        *[asyncio.to_thread(stream_one, base_url, model, p, max_tokens) for p in prompts],
        return_exceptions=True)
    elapsed = time.perf_counter() - started
    errors = [r for r in raw_results if isinstance(r, BaseException)]
    if errors:
        raise RuntimeError(f"{len(errors)}/{concurrency} streams failed the window "
                            f"assertion or errored: {errors[0]!r}")
    results = raw_results
...
async def main():
    ...
    ap.add_argument("--max-tokens", type=int, default=128,
                     help="decode window; pinned via min_tokens=max_tokens+ignore_eos")
    ap.add_argument("--repeat", type=int, default=5, help="how many sequential trials (unique nonces)")
    ap.add_argument("--output", default=None, help="optional path to append one JSON line per trial")
    args = ap.parse_args()
    rows = []
    out_fh = open(args.output, "a") if args.output else None
    for rep in range(args.repeat):
        case = await run_case(args.base_url, args.model, args.prompt, args.concurrency, rep, args.max_tokens)
        rows.append(case)
        if out_fh:
            slim = {k: v for k, v in case.items() if k != "requests"}
            slim["prompt_target"] = args.prompt
            slim["max_tokens"] = args.max_tokens
            slim["rep"] = rep
            out_fh.write(json.dumps(slim) + "\n")
            out_fh.flush()
