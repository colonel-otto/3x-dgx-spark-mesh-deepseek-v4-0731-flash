...
> this page distrusted has since been **independently corroborated**, and the config
> contradiction it flagged is now **resolved**. Read this box, then the original.
>
> ### 1. The declining prefill rate is real
>
> This page called its own trend untrustworthy (n=1, hex content, no fabric gate). A
> matched 70-run depth sweep on the same cluster —
> [`../20260826-decode-depth-2v3/`](../20260826-decode-depth-2v3), median of 7, prose
> content, fabric gated 33/0, TTFT measured server-adjacent — produces the same shape:
>
> | source | content | n | depth | prefill tok/s |
> |---|---|---:|---:|---:|
> | depth sweep | prose | 7 | 8,081 | **2,419** |
> | depth sweep | prose | 7 | 32,268 | **2,206** |
> | depth sweep | prose | 7 | 129,006 | **1,673** |
> | depth sweep | prose | 7 | 257,993 | **1,421** |
> | *this page* | *hex* | *1* | *81,124* | *1,935* |
> | *this page* | *hex* | *1* | *317,904* | *1,373* |
> | *this page* | *hex* | *1* | **967,286** | ***700*** |
>
> Two harnesses, two content types, two sessions, one curve. **Prefill rate declines
> monotonically with depth** — roughly 2,400 tok/s at 8K down to ~700 near the 1M ceiling.
> The absolute values here still should not be quoted (see the original caveats), but
> **the trend is no longer in doubt.**
>
> The interleaving is also informative: this page's hex numbers sit slightly *below* the
> prose curve at comparable depths (1,935 @81K vs ~1,800 interpolated; 1,373 @318K vs
> ~1,350 interpolated). That is close enough that content type looks like a second-order
> effect, not the explanation for the decline.
>
> ### 2. This does NOT disturb prefill parity between 2 and 3 nodes
>
> The original banner was right to fence this off, and the fence holds — but for a
> sharper reason than "the numbers are bad." **Prefill parity is a 2-node-vs-3-node
> claim; this page has no 2-node arm.** A declining rate with depth says nothing about
> node count, because both configurations decline.
>
> The depth sweep measured both arms and found the third node **costs** TTFT at depth
> (+6.5% at 129K, +14.6% at 258K) while being *faster* below 32K. So the honest statement
> is: prefill is at parity to 32K, and slightly favours two nodes past 100K. Neither
> reading is threatened by this page.
>
> ### 3. The `NCCL_IB_HCA` contradiction: possibility (1) confirmed
>
> This page asked whether `HANDOFF.md` §4b is stale. **It is.** §4b's own closing
> sentence names the precondition — *"give the `roceP2p` pair stable IPv4 addressing and
> routing first"* — and that was done on 2026-08-25. Verified on the live cluster:
>
> - all six fabric controllers hold unique `/30` addresses (`192.168.110–112.x`)
> - **zero** `IBV_WC_RETRY_EXC_ERR`, **zero** `fe80::` GIDs, **zero** `GID table changed`
> - 408-request soak with 0 RDMA counter deltas across 132 counters ([#17](../../issues/17))
>
> So the answer is **(1) §4b predates the fabric fix**, not (2) an intermittent
> load-dependent wedge. There is **no live production risk here**. §4b has been retitled
> SUPERSEDED and keeps its failure signature as a diagnostic.
>
> **The `==` is also not a typo** — a leading `=` in `NCCL_IB_HCA` means *exact device
> match* rather than prefix match. Verified from the engine log. Do not "fix" it.
>
> ### What this page is good for
>
> The ~967K-token depth is still **untested anywhere else in this repo**, and it
> demonstrates something worth knowing: **the cluster serves a request at 92% of its
> context ceiling without incident.** That is a real capability datapoint, even though
> the throughput figure attached to it is soft.

> ⚠ **ORIGINAL BANNER (2026-08-26, kept verbatim) — PRELIMINARY, NEEDS MORE
> INVESTIGATION. Do not cite these tok/s numbers as
