...
|---|---:|---|---|
| [20260901T174000Z-eugr-tp2-2node](20260901T174000Z-eugr-tp2-2node/) | 2 | `PRESENT-PASS` (zero JIT misses) | **2-Node TP=2 Eugr Benchmark Suite.** Concurrency sweep ($c \in \{1, 4, 8, 16\}$: 71.9 / 47.2 / 37.3 / 15.7 tok/s decode; 60.0 / 144.3 / 218.1 / 199.9 tok/s aggregate), prompt-effect pair (`code-brief` 77.3 vs `dense-prose` 44.3 tok/s = 1.74x), and 8K context (42.0 tok/s). Completes the $2 \times 2$ cross-engine parity matrix. |
...
| [20260831T0601Z-staggered-spec-acceptance](20260831T0601Z-staggered-spec-acceptance/) | 3 | Inherited (same boot as the gated K sweep) | **Staggered ragged-context correctness gate on the serving config — PASS.** 150 Poisson-arrival requests (30 per tier, c=1..32) with log-uniform prompt depths spanning 1.1K–128K tokens: **0 errors, 0 window failures, 0 preemptions**, and MTP acceptance flat at **40.0–41.6%** from c=1 to c=32 (floor 30%). Settles that draft acceptance survives async continuous batching, the KV allocator is correct under ragged churn, and the virtual-TP zero-fill sink is stable under dynamic compaction. **⚠️ Not a throughput result** — prefill-bound by construction (58–132× prefill:decode, ~1,460–2,300 tok/s prefill); decode for this config remains the K-sweep numbers |
| [20260831T0000Z-eugr-remaining-cells](20260831T0000Z-eugr-remaining-cells/) | 3 | Inherited (same boot as the gated K sweep) | **The last three A/B cells on the winning eugr config.** 131K decode 90.5 vs 83.5 (+8 %, TTFT 53.7 s vs 138 s); prompt-effect pair on the *exact* original prompts (dense-prose recovered from git history) 91.0 / 49.2 vs 81.8 / 49.4; deep 4×200K TTFT 224 s vs ~870 s, still unusable. Completes the cross-engine table |
...
| [20260831T0030Z-eugr-remaining-cells](20260831T0030Z-eugr-remaining-cells/) | 3 | **PASS 24/0** before and after the run | **The three A/B cells the sweep left open.** **Prompt effect is real and *larger* here: code-brief 89.4 vs dense-prose 45.9 = 1.95×** (anemll 1.65×) on identical hardware minutes apart — no decode number means anything without its prompt shape. ⚠ the dense-prose prompt is a **reconstruction** (`ours-bench.py` was never committed), so only the *within-engine* ratio is sound, not the cross-engine dense-prose value. **131K decode 42.3 cold vs anemll 83.5** — but *not* a matched config (`max_model_len` 460800 vs 1048576) and prefill is **2.6× faster** (TTFT 53.7s vs 138.1s). A first attempt was discarded when the prefix cache served later reps. **4×200K: 1.4 tok/s, TTFT 227s, 0 errors** — completes, still unusable |
| [20260831T1000Z-matched-engine-ab](20260831T1000Z-matched-engine-ab/) | 3 | Absent (engine-swap A/B never frees the GPUs; recorded ABSENT, not inherited) | **The matched cross-engine A/B — the comparison every earlier eugr-vs-anemll number was standing in for.** Both engines back-to-back on 2026-08-31, same harness, same prompt, same 128-tok window, median-of-5, `max_num_seqs=16` on **both** arms: one variable. **eugr wins every aggregate cell +31% to +61%** (c=8 154.8→249.9) and **single-stream decode +38%** (61.5→84.7). The earlier table *understated* eugr: it compared against **2026-08-21** anemll rows, and its single-stream deltas (+5/+8/+11%) all sat inside the repo's 12% parity tolerance. anemll keeps two things — **1.86× the KV pool** (4.39M vs 2.36M; `nvfp4_ds_mla` is refused on MLA in the eugr build, so this is permanent) and per-stream decode at the c=16 cap (18.2 vs 15.0, where eugr still moves +33% more total tokens). anemll was far less stable under load: **84% trial spread at c=16** vs eugr's 14.7%. Correctness 7/7 before any throughput cell. Permanent confound: MTP K=2 vs DSpark nst=5 (nst<5 refused by the checkpoint) |
| [20260831T0525Z-eugr-remaining-cells-matched](20260831T0525Z-eugr-remaining-cells-matched/) | 3 | Inherited (same boot as the row above) | **Corrects two cells above on matched harness + exact prompts.** 131K decode **90.5 vs 83.5 (+8 %, TTFT 53.7 s vs 138 s)** on `bench-miaai` — the 42.3 measured the driver's own filler prompt, so "−49 %" was never a comparison; dense-prose **49.2 vs 49.4** on the original prompt (git `b078eb4`, not lost), ratio 1.85×; deep 4×200K confirmed |
